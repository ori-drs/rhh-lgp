...
void createDrawerScene(rai::Configuration &C)
{
	...
	// rai::Frame *f5 = C.addFrame(STRING("cilinder2"), "", "type:ssBox size:[.1 .1 .2 .02] color:[1. 0. 0.], logical={ object, cilinder }");
	// f5->setPosition({1.0, -1.5, 0.3});
	// f5->setQuaternion(noRotationQuaternion.getArr4d());
	//***********************************************************************************************************************
	C.proxies.clear();
