	...
	for (rai::Frame *f : C.frames)
	{
		if (f->joint && f->joint->active)
		{
			if (f->joint->type == rai::JT_free)
				f->setJoint(rai::JT_rigid); // make free joints rigid --> this is the only change to the end config
			else if (f->joint->active && f->joint->type != rai::JT_rigid)
				C.activeJoints.append(f->joint); // append active joints
		}
	}
	C.checkConsistency();
}
...
void RHHLGP_solver::getStats(BoundType bound, shared_ptr<KOMO> komo)
{
	...
	cout << "ITERATION RESULTS: "
		 << "; COSTS: " << cost_here << "; CONSTRAINTS: " << constraints_here << endl;
}
...
Skeleton LGP_Node::getSkeleton(bool finalStateOnly, uint from, uint to) const
{
	...
	if (!finalStateOnly)
	{
		...
		{ //< only take entries between in [from,to]
	...
	for (uint k = 0; k < states.N; k++)
	{
		...
		for (uint i = 0; i < G.N; i++)
		{
			...
			{
				Node *n = G(i);
				if (n->isGraph() && n->graph().findNode("%decision"))
					continue; // don't pickup decision literals
				StringA symbols;
				...
				if (!rai::Enum<SkeletonSymbol>::contains(symbols.first()))
					continue;
	...
	for (uint i = 0; i < skeleton.N; i++)
	{
		...
		if (skeletonModes.contains(se.symbol))
...
Skeleton LGP_Node::getNextHorizonSkeleton(intA window) const
{
	...
	std::cout << "window.first() " << window.first() << "\n";
	std::cout << "window.last() " << window.last() << "\n";
	...
	for (uint i = 0; i < S_now.N; ++i)
	{
		...
		// std::cout << "se.phase1 " << se.phase1 << "\n";
		// std::cout << "st.phase0 " << st.phase0 << "\n";
		// std::cout << "st.phase1 " << st.phase1 << "\n";
		// std::cout << "se symbol" << se.symbol << "\n";
		// std::cout << "st symbol " << st.symbol << "\n";
...
void LGP_Node::optBound3(BoundType bound, bool collisions, intA window, int verbose)
{
	...
	if (komo->verbose > 1)
		komo->reportProblem();
	if (komo->verbose > 5)
		komo->animateOptimization = komo->verbose - 5;

	try
	{
		if (bound != BD_poseFromSeq)
		{
			if (bound == BD_seqPath && verbose > 3)
				cout << "SKELETON FOR SEQ PATH: " << S << endl;
			komo->run();
		}
		else
		{
	...
	if (bound == BD_pose)
	{
		...
		if (parent)
			bound_cost += parent->cost(bound); // this is sequentially additive cost
	}										   /*else {
	...
	if (feas)
	{
		if (count(bound) == 1 || bound_cost < highestBound)
			highestBound = bound_cost;
	}
...
void LGP_Tree::optBestOnLevel(BoundType bound, LGP_NodeL &drawFringe, BoundType drawFrom, LGP_NodeL *addIfTerminal, LGP_NodeL *addChildren, const intA window)
{ // optimize a seq
	if (!drawFringe.N)
	{
	...
	if (n && !n->count(bound))
	{
		...
		{ // use path bound if seq path does not work
...
void LGP_Tree::step(int horizon, int windowN)
{
	...
	if (fringe_path.N)
		cout << "EVALUATING PATH " << fringe_path.last()->getTreePathString(horizon * windowN, horizon * (windowN + 1));
...
void LGP_Tree::run2(int windowN, int horizon, uint steps)
{
	...
	for (uint k = 0; k < steps; k++)
	{
		...
		if (COUNT_time > stopTime)
			break; //< count time is counting the entire opt time so we dont want this here
	}
...
void RHHLGP_solver::setupLGP(LGP_Tree &tree, const char *priorDecisions) const
{
	...
	if (priorDecisions)
		tree.walkToNode(priorDecisions); // this adds the previous decisions to the fol of the new tree
}
...
void RHHLGP_solver::optimize(ptr<OpenGL> gl)
{
	...
	cout << endl
		 << "SOLUTION SEQ: " << solution << endl;
	cout << "OVERALL RESULTS ===> "
		 << "SOLUTION LEN: " << solutionLen << "; #steps: " << i - 1 << "; #OF EVALS: " << totalExpansions << "; #TREE NODES: " << COUNT_node << "; TOTAL TIME: " << COUNT_time << endl
		 << endl;
}
