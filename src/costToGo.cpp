...
void costToGo1And2(LGP_Node *n, rai::String target, StringA effectors, int numTargets)
{
	...
	{
	case str2int("step"):
		n->cost(BD_symbolic) += stepHeuristic(n, decision, STRING(target).p, 1);
		break;

		// if you can, connect is good
	case str2int("connect"):
		n->cost(BD_symbolic) += connectHeuristic(n, decision) - 1.5;
		break;

		// always go to the tray if connected
	case str2int("steptogether"):
		n->cost(BD_symbolic) += stepHeuristic(n, decision, STRING(target).p, 2);
		break;

		// dont disconnect until we are on the tray and not if we just connected
	case str2int("disconnect"):
		n->cost(BD_symbolic) += disconnectHeuristic(n, decision);
		break;

		// dont grasp if it is too far away
	case str2int("grasp"):
		n->cost(BD_symbolic) += graspHeuristic(n, decision);
		break;
	};
}
...
void costToGo3(LGP_Node *n, StringA targets, char *effector)
{
	...
	if (n->step == 0)
	{
		n->cost(BD_symbolic) = euclideanDistance(n->startKinematics.getFrame(STRING("R_handA"))->getPosition(), n->startKinematics.getFrame(STRING(effector))->getPosition()) + std::min(euclideanDistance(n->startKinematics.getFrame(targets.elem(0))->getPosition(), n->startKinematics.getFrame(STRING(effector))->getPosition()),
	...
	case (str2int("step")):
	{
		...
		if (decision->substitution.elem(-3)->key.getFirstN(1) == STRING("R"))
			otherCrawler = "L";
		rai::String supportObject = getSupportObject(n->parent, STRING(otherCrawler << "_handA"));
		if (!supportObject.N)
			supportObject = getSupportObject(n->parent, STRING(otherCrawler << "_handB"));
	...
	case str2int("connect"):
		n->cost(BD_symbolic) += connectHeuristic(n, decision);
		break;
...
void costToGo4(LGP_Node *n, int numObjects, rai::String source, rai::String target)
{
	...
	case str2int("connect2mobile"):
		n->cost(BD_symbolic) -= 0.15;
		break;
	case str2int("connect"):
		n->cost(BD_symbolic) += connectHeuristic(n, decision) - .15;
		break;

		// only place on target
	case str2int("place"):
		n->cost(BD_symbolic) += placeHeuristic(n, decision, target);
		break;
...
void costToGo6(LGP_Node *n, int numObjects, rai::String target)
{
	...
	// only place on target
	case str2int("place"):
		n->cost(BD_symbolic) += placeHeuristic(n, decision, target, true);
		break;

	// only place on target
	case str2int("placeOnGoal"):
		n->cost(BD_symbolic) += placeHeuristic(n, decision, target, true);
		break;

	// check if connected to mobile base -- if yes, then we skip this heuristic because we can move freely
	case str2int("grasp"):
		n->cost(BD_symbolic) += graspHeuristic(n, decision, true);
		break;
	};
}
...
void LGP_Tree::seqBoundCostToGo(LGP_Node *node)
{
	...
	if (node && !node->count(bound))
	{
		...
		if (node->feasible(bound))
		{
			...
			if (addIfTerminal && node->isTerminal)
				addIfTerminal->append(node);
...
{
	if (node->decision && node->parent->decision)
	{
		rai::String currentDecision = std::dynamic_pointer_cast<const FOL_World::Decision>(node->decision)->rule->key;
		rai::String parentDecision = std::dynamic_pointer_cast<const FOL_World::Decision>(node->parent->decision)->rule->key;
		if (parentDecision == "pick")
		{
			if (currentDecision == "place")
				node->cost(BD_symbolic) += 5;
			else if (currentDecision == "pick")
				node->cost(BD_symbolic) += 0;
			else
				node->cost(BD_symbolic) += 5;
		}
